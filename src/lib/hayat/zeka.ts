/* Hayat Simülatörü — yapay zekâ çekirdeği (ortamdan bağımsız)

   Buradaki her şey saf: istemler (prompt), model çıktısını temizleyen
   süzgeçler ve oyun kurallarına çeviren mantık. Modele SORUYU KİMİN
   sorduğu dışarıdan verilir (Sorucu):

     - Sunucuda: hayat.server.ts, ortam değişkenindeki anahtarla sorar.
     - Tarayıcıda: zeka-tarayici.ts, oyuncunun kendi anahtarıyla sorar.

   İki yol da aynı istemleri ve aynı temizleyicileri kullanır; davranış
   nerede çalıştığından bağımsız olarak birdir. */

import { EVRE_ADI, HEDEFLER, evreBul } from "./veri";
import { iliskiBul, yetiskinIcerikAcik } from "./motor";
import { ETIKET_LISTESI, ozellikOrtami, profilKur } from "./profil";
import type { Durum, Etiket, Etki, Olay, Ozellik, Secenek, StatAnahtar } from "./tipler";

/**
 * Modele tek bir soru sorar ve JSON nesnesi döndürür.
 * `gerekli` içindeki alanlar boşsa hata fırlatması beklenir.
 */
export type Sorucu = (
  system: string,
  user: string,
...
export async function zSahneUret(
  sorucu: Sorucu,
  durum: Durum,
  kacinilan: string[],
...
export async function zProfilCoz(
  sorucu: Sorucu,
  ad: string,
  tur: "kisilik" | "meslek",
): Promise<Ozellik | null> {
...
export async function zIsimOner(
  sorucu: Sorucu,
  cinsiyet: string,
  kacinilan: string[],
): Promise<string | null> {
...
export async function zAlanOner(
  sorucu: Sorucu,
  tur: "meslek" | "kisilik",
...
export async function zSerbestCevap(
  sorucu: Sorucu,
  metin: string,
  olay: Olay,
  durum: Durum,
...
Bunun sonucunu yaz.`;
  ...
  try {
    ...
    console.error("[zSerbestCevap]", err);
    return null;
  }
}
